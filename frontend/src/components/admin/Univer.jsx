...
const Univer = () => {
    ...
    async function getStudents(){
      ...
      const [   dataArrayy
        , setDataa] = useState([]);
          ...
          async function getStudentsss(address){
              if (typeof window.ethereum !== "undefined") {
                const provider = new ethers.providers.Web3Provider(window.ethereum)
                console.log("hi")
                await provider.send('eth_requestAccounts', [])
                ...
                try {
                  console.log("hi")
                  console.log(address)
                  // const transactionResponse = await contract.getUniStudowner(address)
  ...
  >
       ...
       <button className='button'  onClick={()=>getStudents()}>UNIVERSITIES</button></Typography>
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell>GET STUDENTS</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
        {dataArray.map((row) => (
            <TableRow key={row}>
              ...
              <TableCell> <button className='button'  onClick={()=>getStudentsss(row.address)}>getSTUDENT</button></TableCell>
            </TableRow>
          ))}
